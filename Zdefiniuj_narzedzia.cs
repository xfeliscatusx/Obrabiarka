...
{
    public partial class Zdefiniuj_narzedzia : MetroFramework.Forms.MetroForm
    {
        ...
        List<string> compareName = new List<string>() { "Wiertła", "Frezy", "Narzędzia tokarskie" };
        ...
        string x, y, z;
        Kryteria3 kryteriaXYZ = new Kryteria3();
        Kryteria4  kryteriaXC = new Kryteria4();
        ...
        private void Obrabiarki_Load()
        {
            ...
            {
                GroupOfMachineId = dt.Rows[i]["id_narzedzia"].ToString();
                NameOfMachineGroup = dt.Rows[i]["grupa_narzedzi"].ToString();
                GroupOfMachine = new TreeNode(NameOfMachineGroup);
                CmdString = "SELECT id_rodzajenarzedzi, rodzaj_narzedzia FROM RodzajeNarzedzi WHERE id_narzedzia=@id_narzedzia";
                cmd = new SqlCommand(CmdString, con);
                cmd.Parameters.AddWithValue("@id_narzedzia", GroupOfMachineId);
                sda = new SqlDataAdapter(cmd);
                dt2 = new DataTable();
                sda.Fill(dt2);
                for (int j = 0; j < dt2.Rows.Count; j++)
                {
                    TypeOfMachineId = dt2.Rows[j]["id_rodzajenarzedzi"].ToString();
                    NameOfMachineType = dt2.Rows[j]["rodzaj_narzedzia"].ToString();
        ...
        private string treeviewquery()
        {
            ...
            {
                query += string.Format("id_narzedzia='{0}' OR ", generatedList[i]);
            }
            zapytanieDoDrzewa = "SELECT id_narzedzia, grupa_narzedzi FROM Narzedzia Where " + query;
        ...
        private void compareNameMethod()
        {
            ...
            {
                if (tickList[i] == "True")
                    generatedList.Add((i + 1).ToString());
            }
        }
        ...
        private void Zdefiniuj_narzedzia_Load(object sender, EventArgs e)
        {
        ...
        void kryteriaXCf()
        {
            x = kryteriaXC.returnValueX();
            y = kryteriaXC.returnValueY();
            z = kryteriaXC.returnValueZ();
            createListOfParameters();
        ...
        private void kryteriaWyboruToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (value == "Narzędzia tokarskie")
            {
                kryteriaXC.eventList += new Kryteria6(kryteriaXCf);
            ...
            {
                kryteriaXYZ.eventList += new Kryteria5(kryteriaXYZf);
        ...
        private void joinTuple()
        {
            ...
            for (int i = 0; i < finishTupleListKryteria.Count; i++)
            {
                ...
                query += string.Format("insert into [dbo].[ZdefiniowaneNarzedzia]" +
                    " (node_number, nazwa_zabiegu, rodzaj_narzedzia, waga, d1, l,f)" +
                    " Values('{0}', '{1}', '{2}', '{3}', '{4}','{5}','{6}') ",
                    result, operationName, rodzaj_obrabiarki, wagi, x, y, z);
        ...
        private void truncateTable()
        {
            ...
            {
                sqlCon.Open();
                string CmdString = "TRUNCATE TABLE [dbo].[ZdefiniowaneNarzedzia];";
                cmd = new SqlCommand(CmdString, sqlCon);
                sqlDa = new SqlDataAdapter(cmd);
                dtbl = new DataTable();
                sqlDa.Fill(dtbl);
